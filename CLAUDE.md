...
- [app/api/analyze-professor/route.ts](app/api/analyze-professor/route.ts) — takes `{ documents: {fileName, text}[] }` (already-extracted text, one entry per file), concatenates them, and calls OpenAI with a fixed Structured Outputs schema (`topics`/`examStyle`/`assignmentStyle`) to summarize patterns across everything a given professor has produced. Drives the "교수님" tab: documents are uploaded per-professor via `/api/extract` and persisted (with their extracted text) in the `documents` table so re-analysis never needs the files re-uploaded.
...
Two more tables back the "교수님" tab (per-professor document collection, own-rows-only via RLS, no cross-user sharing): `professors` (`user_id`, `name`, `school`, `department`, `created_at`) and `documents` (`user_id`, `professor_id` (nullable FK → `professors.id`, `on delete cascade`), `file_name`, `format`, `content`, `created_at` — unlike `document_uploads`, this table stores the actual extracted text so `/api/analyze-professor` can re-run over everything a professor has accumulated without re-uploading). Migration: [supabase/migrations/20260727_create_professors_and_documents.sql](supabase/migrations/20260727_create_professors_and_documents.sql).
