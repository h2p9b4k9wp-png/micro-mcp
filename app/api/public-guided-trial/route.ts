...
import { getClientIp, checkGuidedTrialUsed, recordAnonymousUsage } from '@/lib/anonymous-usage';
...
//
// 기존 /api/public-analyze(파일 분석, 시간당 5회/일일 15회)와는 남용 방지 방식이 다릅니다 —
// 이 체험은 IP당 평생 딱 1회만 허용합니다(lib/anonymous-usage.ts의 checkGuidedTrialUsed,
// kind='guided'는 시간 창 없이 행이 하나라도 있으면 이미 쓴 것으로 판단). 한 번 다 쓰면
// 계정을 만들어야 다시 체험할 수 있다는 게 의도라, 시간당/일일 예산과는 별도로 관리합니다.
//
...
export async function POST(req: Request) {
  try {
    ...
    const alreadyUsed = await checkGuidedTrialUsed(supabaseAdmin, ip);
    if (alreadyUsed) {
    ...
    await recordAnonymousUsage(supabaseAdmin, ip, 'guided');
