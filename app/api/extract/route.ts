...
// 실측) 실행 시간을 늘려 잡습니다.
//
// ⚠️ 메모리는 여기서 지정할 수 없고 **Vercel 프로젝트 설정(Settings → Functions)**에서
// 올려야 합니다. vercel.json의 functions 블록으로도 지정할 수 있게 되어 있지만 쓰지 않습니다 —
// 한 번 넣었다가 배포 자체가 트리거되지 않는 문제를 겪었고, 실행시간은 이 라우트 세그먼트
// 설정만으로 충분해서 잘못될 수 있는 곳을 하나 줄이는 쪽을 택했습니다. 메모리가 기본값이면
// 큰 PDF는 상한 안이어도 실패합니다.
