...
import { CIRCUIT_VIEWBOX, getPipelinePosition, buildCircuitTracePath } from '@/lib/circuit/geometry';
...
export function CircuitBoard({ graph, onNodeClick }: CircuitBoardProps) {
  ...
  const positionOf = (nodeId: NodeId) => {
    for (const layer of LAYERS) {
      const idx = nodesByLayer[layer].findIndex((n) => n.id === nodeId);
      if (idx !== -1) return getPipelinePosition(layer, idx, nodesByLayer[layer].length);
    }
    return null;
  };
  ...
  return (
    <div>
      ...
      <div className="bg-[#0D0B11] rounded-2xl border border-[#2A2632] p-3 sm:p-6 mb-6 shadow-sm overflow-x-auto">
        <div className="hidden sm:grid grid-cols-3 max-w-[720px] mx-auto text-center text-[11px] font-semibold text-[#5B5566] uppercase tracking-wide mb-2">
          {LAYERS.map((layer) => (
        ...
        >
          ...
          >
            {graph.edges.map((edge) => {
              ...
              const d = buildCircuitTracePath(from.x, from.y, to.x, to.y);
              return (
                <Wire
                  key={edgeKey(edge)}
                  d={d}
