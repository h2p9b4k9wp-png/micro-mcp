...
// 모든 항목은 evidence(그 항목의 근거가 되는 원문 발췌)를 함께 들고 있습니다 — COMMON_RULES 참고.
...
4) evidence 필드가 있는 항목은 그 항목의 근거가 되는 원문 그대로의 짧은 발췌를 evidence에 넣으세요. 원문에서 발췌를 찾을 수 있는 한 그 항목을 포함하세요 — 발췌가 정말로 없을 때만 그 항목을 제외하세요. (evidence 필드가 없는 요약성 필드는 발췌 없이 문서 내용에 충실하게 쓰면 됩니다.)
    ...
    items: {
      ...
      items: {
        ...
        properties: {
          ...
          evidence: {
            type: 'string',
            description: '이 질문이 겨냥하는 약점의 근거가 되는, 원문에서 그대로 발췌한 문구',
          },
        },
        required: ['question', 'targetWeakness', 'draftAnswer', 'evidence'],
      },
    },
  },
  required: ['items'],
